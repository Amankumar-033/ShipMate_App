...
//   // New: suggestions & selected flag
//   const [suggestions, setSuggestions] = useState<string[]>([]);
//   const [selectedSuggestion, setSelectedSuggestion] = useState<string | null>(null);

//   const { addEstimate } = useAppContext();

//   // Debounced ORS autocomplete for steps 0 & 1
//   useEffect(() => {
//     if (step > 1) {
//       setSuggestions([]);
//       return;
//     }
//     const query = fields[step];
//     if (!query.trim()) {
//       setSuggestions([]);
//       return;
//     }
...
//     if (step === 2 && (isNaN(Number(fields[2])) || Number(fields[2]) <= 0))
...
//       setError("Please pick one of the dropdown suggestions.");
//       return;
//     }
...
//         <AppLogo>
//           <FaShippingFast />{" "}
//           <span style={{ fontWeight: 700, fontSize: "1.1rem", marginLeft: "7px" }}>
//             ShipMate
//           </span>
//         </AppLogo>
...
//                       </SuggestionList>
//                     )}
//                     {step <= 1 && fields[step].trim() !== "" && suggestions.length === 0 && (
...
//                     type="button"
//                     onClick={handleNext}
//                     disabled={!fields[step].trim() || (step <= 1 && !selectedSuggestion)}
//                   >
//                     Next <FaArrowRight />
//                   </NextButton>
//                 )}
...
export default function EstimateForm({ onEstimate }: EstimateFormProps) {
  ...
  // Debounced autocomplete for steps 0 & 1, with guard to keep list hidden after select
  useEffect(() => {
    if (step > 1) {
      setSuggestions([]);
      return;
    }
    // if user just selected this suggestion, do nothing
    if (selectedSuggestion === fields[step]) {
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const mult =
        shippingOption === "Express" ? EXPRESS_MULTIPLIER : STANDARD_MULTIPLIER;
      let totalCost = calculateCost(distanceKm, +weight, mult);
      if (currency !== DEFAULT_CURRENCY) {
        totalCost = await convertCurrency(totalCost, currency);
      }
      const est: Estimate = {
  ...
  return (
    ...
    >
      <GlassCard initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }}>
        <AppLogo>
          <FaShippingFast />{" "}
          <span style={{ fontWeight: 700, fontSize: "1.1rem", marginLeft: 7 }}>
            ShipMate
          </span>
        </AppLogo>
